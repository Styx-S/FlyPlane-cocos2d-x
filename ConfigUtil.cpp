...
#define config UserDefault::getInstance()
	...
	if (!UserDefault::isXMLFileExist()) {
		...
		config->setFloatForKey("CREATE_SORMENEMYBYBIGENEMY_DELAY", CREATE_SORMENEMYBYBIGENEMY_DELAY_DEFAULT);
	...
	this->m_floatMap["CREATE_SORMENEMYBYBIGENEMY_DELAY_DEFAULT"] = config->getFloatForKey("CREATE_SORMENEMYBYBIGENEMY_DELAY_DEFAULT");
